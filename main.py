...
def create_database():
    try:
        ...
        connection.commit()
        print("Done.")
        if input("Would you change current database to this new database(yes/no): ").lower() in ("yes", "y"):
            query = f"""USE {db_name}"""
            cursor.execute(query)
...
def create_table():
    try:
        ...
        second_column_name = input("first column name: ")
...
def show_some_info():
    print("1. show all database")
    print("2. show all table in current database.")
    print("3. show all columns in some table.")
    print("4. show all data in some table.")
    print("5. show other info.")
    print("6. Back.")
    ...
    with pymysql.connect(host="localhost", port=3307, user="root", password="") as connection:
        ...
        with connection.cursor() as cursor:
            ...
            while True:
                ...
                print("4. Insert info to table.")
                print("5. Delete info from table.")
